...
// Class names for button states
const BTN_CLASSES = {
  BASE: "px-3 py-1.5 rounded-lg border text-xs font-medium transition-all duration-300",
  ACTIVE:
    "text-primary border-primary shadow-[0_0_15px_rgba(255,107,107,0.4)] bg-card-dark",
  INACTIVE:
    "border-card-border bg-card-dark text-gray-400 hover:text-primary hover:border-primary hover:shadow-[0_0_15px_rgba(255,107,107,0.4)]",
};
...
const state = {
  ...
  sortMode: SORT_MODES.RECOMMENDED,
  searchQuery: "",
  visibleCount: 5,
};
...
function sortItems(mode) {
  ...
  switch (mode) {
    ...
    case SORT_MODES.ALPHABETICAL:
      state.items = [...initialResources].sort((a, b) =>
        a.title.localeCompare(b.title)
      );
      break;
    case SORT_MODES.RANDOM:
      state.items = shuffleArray(initialResources);
      break;
  }
...
function createResourceCard(resource) {
  return `
    <article class="group relative flex flex-col md:flex-row gap-6 border-b border-card-border py-6 transition-colors hover:bg-white/[0.02] -mx-4 px-4 sm:mx-0 sm:px-0" role="listitem">
      <div class="shrink-0">
        <div
          class="h-[100px] w-[100px] rounded-lg bg-cover bg-center ring-1 ring-white/10"
          style="background-image: url('${resource.imageUrl}')"
          role="img"
          aria-label="${resource.title} 썸네일"
        ></div>
      </div>
      <div class="flex flex-1 flex-col justify-center gap-2">
        <div class="flex items-center justify-between">
          ...
          <span class="material-symbols-outlined text-gray-500 group-hover:text-primary transition-colors text-xl" aria-hidden="true">
            arrow_outward
          </span>
        </div>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        aria-label="${resource.title} 방문하기"
...
// Render Function
function render() {
  const { listContainer, noResults, loadMoreContainer } = elements;
  ...
  // Render Items
  listContainer.innerHTML = visible.map(createResourceCard).join("");
...
function updateSortButtons() {
  const buttons = [
    { el: elements.btnRecommend, mode: SORT_MODES.RECOMMENDED },
    { el: elements.btnAlpha, mode: SORT_MODES.ALPHABETICAL },
    { el: elements.btnRandom, mode: SORT_MODES.RANDOM },
  ];

  buttons.forEach(({ el, mode }) => {
    if (!el) return;
    const isActive = state.sortMode === mode;
    el.className = `${BTN_CLASSES.BASE} ${
      isActive ? BTN_CLASSES.ACTIVE : BTN_CLASSES.INACTIVE
    }`;
...
function init() {
  ...
  elements.loadMoreBtn = document.getElementById("btn-load-more");
  elements.searchInput = document.getElementById("search-input");
  elements.btnRecommend = document.getElementById("btn-sort-recommended");
  elements.btnAlpha = document.getElementById("btn-sort-alpha");
  elements.btnRandom = document.getElementById("btn-sort-random");
  ...
  if (elements.btnRecommend) {
    elements.btnRecommend.addEventListener("click", () =>
      sortItems(SORT_MODES.RECOMMENDED)
    );
  }

  if (elements.btnAlpha) {
    elements.btnAlpha.addEventListener("click", () =>
      sortItems(SORT_MODES.ALPHABETICAL)
    );
  }

  if (elements.btnRandom) {
    elements.btnRandom.addEventListener("click", () =>
      sortItems(SORT_MODES.RANDOM)
    );
  }
